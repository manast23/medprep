...
- **- ✅ **Anatomy `reference_url` backfill — COMPLETE.** All 105 Anatomy questions now have a verified StatPearls/NCBI Bookshelf source (one uses Radiopaedia — the neck lymph-node-levels question). Each URL was individually matched to the specific fact tested (not just the general topic) via live web search, with the article content checked against the question/answer before assigning. ✅ **Physiology `reference_url` backfill — COMPLETE.** All remaining 61 Physiology questions (Cardiovascular, Nervous System, Respiratory, Renal & Body Fluids, Endocrinology) now have a verified StatPearls/NCBI Bookshelf source, individually matched to the specific fact tested. Physiology's reference_url coverage is now 111/111 (100%).
