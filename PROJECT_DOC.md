...
- **- ✅ **Anatomy `reference_url` backfill — COMPLETE.** All 105 Anatomy questions now have a verified StatPearls/NCBI Bookshelf source (one uses Radiopaedia — the neck lymph-node-levels question). Each URL was individually matched to the specific fact tested (not just the general topic) via live web search, with the article content checked against the question/answer before assigning. Physiology's 50/61 split was left as-is (not part of this pass) — flag if you'd like that backfilled too.
